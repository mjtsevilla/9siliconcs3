...
- My design follows the DRY principle because common ClubMember information and methods are written only once in the parent class. ClubOfficer inherits these features instead of creating duplicate versions of them. the child class only ads duty attribute and performDuty() method that are specific to officers. Using inheritance and super().__init__() therefore reduces repeated code.
